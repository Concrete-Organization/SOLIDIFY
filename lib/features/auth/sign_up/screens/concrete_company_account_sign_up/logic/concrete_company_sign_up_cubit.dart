...
class ConcreteCompanySignUpCubit
    extends Cubit<ConcreteCompanyAccountSignUpState> {
  ...
  void emitConcreteCompanySignUpStates() async {
    emit(
        const ConcreteCompanyAccountSignUpState.concreteCompanySignUpLoading());

    if (commercialLicenseFile == null) {
      emit(
        ConcreteCompanyAccountSignUpState.concreteCompanySignUpError(
          error: ApiErrorModel(
            message: 'Commercial license file is required',
          ),
        ),
      );
      return;
    ...
    final response =
        await _concreteCompanyAccountRepo.companySignUp(requestModel);
